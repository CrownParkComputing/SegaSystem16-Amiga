#!/bin/bash
# Build Alien Syndrome (Sega System 16B, World, UNPROTECTED plain 68000) as a
# user-mode RTG presenter using the Musashi 68000 interpreter. Same proven recipe
# as the working Shinobi port: the real System 16 game logic runs in C emulation,
# shinobi_hwrender.c paints a 320x224 chunky frame, and shinobi_rtg_main.c scales
# it to a Picasso96/uaegfx RTG screen. aliensyn is unencrypted (no FD1094/FD1089).
set -euo pipefail
...
STAGE_DIR="$BUILD_DIR/aliensyn"
OBJ_DIR="$BUILD_DIR/aliensyn_interp_obj"
DIST_DIR="$GAME_DIR/dist"

mkdir -p "$STAGE_DIR" "$OBJ_DIR" "$DIST_DIR"

echo "== build clean Alien Syndrome loader (ROMs are loaded from user-supplied files at runtime) =="

echo "== build Alien Syndrome RTG bezel (aliensyn PNG if present, else procedural, RGB332 indexed, 864x486) =="
python3 "$SRC_DIR/tools/make_aliensyn_rtg_bezel.py" >/dev/null
...
echo "== compile Alien Syndrome interpreter, RTG presenter, and painter =="
...
cp -f "$BUILD_DIR/aliensyn_interp" "$GAME_DIR/AlienSyndrome"
ls -lh "$GAME_DIR/AlienSyndrome"

echo "== build bootable RTG HDF =="
BOOT_DIR="$BUILD_DIR/aliensyn_interp_boot"
rm -rf "$BOOT_DIR"
mkdir -p "$BOOT_DIR/s"
cp -f "$BUILD_DIR/aliensyn_interp" "$BOOT_DIR/AlienSyndrome"
cat > "$BOOT_DIR/s/startup-sequence" <<'EOF'
; Alien Syndrome RTG direct boot, based on the supplied RTG/Picasso96 boot disk.
...
HDF="$DIST_DIR/AlienSyndrome_RTG.hdf"
rm -f "$HDF"
...
cp -f "$BASE_HDF" "$HDF"
xdftool "$HDF" delete AlienSyndrome >/dev/null 2>&1 || true
xdftool "$HDF" delete AlienSyndrome.info >/dev/null 2>&1 || true
...
xdftool "$HDF" write "$BUILD_DIR/aliensyn_interp" AlienSyndrome
xdftool "$HDF" write "$BOOT_DIR/s/startup-sequence" S/startup-sequence

# NOTE: the HDF is created ROM-free on purpose. aliensyn ROMs (epr-11083.a4 etc.)
# are injected separately into roms/aliensyn by the install/import step or the
# shared ROM folder at runtime.

echo "DONE -> $GAME_DIR/AlienSyndrome and $HDF"
