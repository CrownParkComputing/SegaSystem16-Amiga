...
# Alien Syndrome (Sega System 16B, aliensyn) Amiga port. Reuses the port's own
# Musashi interpreter engine (hal/shinobi_interp.c) and RTG software renderer
# (hal/shinobi_hwrender.c, -DSHINOBI_RTG) unchanged, with shim Amiga headers +
# host API stubs so both link natively. Produces build/aliensyn_render.
set -euo pipefail
...
SRCS=(
  ...
  "$SRC_DIR/tools/aliensyn_host_stubs.c"
  "$SRC_DIR/tools/aliensyn_assets_host.c"
  "$SRC_DIR/tools/aliensyn_render.c"
)

echo "== compiling + linking $OUT_DIR/aliensyn_render =="
"$CC" "${CFLAGS[@]}" "${SRCS[@]}" -o "$OUT_DIR/aliensyn_render" -lm
echo "OK -> $OUT_DIR/aliensyn_render"
