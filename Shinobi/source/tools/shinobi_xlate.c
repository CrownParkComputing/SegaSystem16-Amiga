...
int xl_emit_term(uint32_t pc, const xdec *d, uint32_t fall,
    ...
    case XT_JMP_IND: case XT_JSR_IND:
        ...
        if (d->tmode==2){               /* (An): target = aN                   */
            ...
            if(!put32(out,&n,cap,env->gregs_pc))return 0;
        } else if (d->tmode==6){        /* (d8,An,Xn): target = aN + d8 + Xn */
            uint16_t e = d->w1;
            int idx_a    = (e >> 15) & 1;
            int idx_reg  = (e >> 12) & 7;
            int idx_long = (e >> 11) & 1;
            ...
            if (idx_a || scale != 0) return 0;         /* Dn only for now */
            if(!put16(out,&n,cap,0x23C8 | d->treg))return 0;       /* move.l aN,(g_pc).L */
            if(!put32(out,&n,cap,env->gregs_pc))return 0;
            if (disp8) {
                if(!put16(out,&n,cap,0x06B9))return 0;             /* addi.l #d8,(g_pc).L */
                if(!put32(out,&n,cap,(uint32_t)(int32_t)disp8))return 0;
                if(!put32(out,&n,cap,env->gregs_pc))return 0;
            }
            if (idx_long) {
                if(!put16(out,&n,cap,(uint16_t)(0xD1B9 | (idx_reg<<9))))return 0; /* add.l Dn,(g_pc).L */
                if(!put32(out,&n,cap,env->gregs_pc))return 0;
            } else {
                if(!put16(out,&n,cap,(uint16_t)(0x2F00 | idx_reg)))return 0;      /* move.l Dn,-(a7) */
                if(!put16(out,&n,cap,(uint16_t)(0x48C0 | idx_reg)))return 0;      /* ext.l Dn */
                if(!put16(out,&n,cap,(uint16_t)(0xD1B9 | (idx_reg<<9))))return 0; /* add.l Dn,(g_pc).L */
                if(!put32(out,&n,cap,env->gregs_pc))return 0;
                if(!put16(out,&n,cap,(uint16_t)(0x201F | (idx_reg<<9))))return 0; /* move.l (a7)+,Dn */
            }
        } else {
            return 0;
        }
