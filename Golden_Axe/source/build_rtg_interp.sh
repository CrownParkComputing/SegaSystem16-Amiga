#!/bin/bash
# Build Golden Axe as a user-mode RTG presenter using the Musashi 68000 interpreter.
...
STAGE_DIR="$BUILD_DIR/golden_axe"
OBJ_DIR="$BUILD_DIR/golden_axe_interp_obj"
DIST_DIR="$GAME_DIR/dist"

mkdir -p "$STAGE_DIR" "$OBJ_DIR" "$DIST_DIR"

echo "== build clean Golden Axe loader (ROMs are loaded from user-supplied zip/files at runtime) =="

echo "== build Golden Axe RTG bezel (Bezel Project artwork, RGB332 indexed, 864x486) =="
...
echo "== compile Golden Axe interpreter, RTG presenter, and painter =="
...
cp -f "$BUILD_DIR/shinobi_interp" "$GAME_DIR/GoldenAxe"
ls -lh "$GAME_DIR/GoldenAxe"

echo "== build bootable RTG HDF =="
BOOT_DIR="$BUILD_DIR/golden_axe_interp_boot"
rm -rf "$BOOT_DIR"
mkdir -p "$BOOT_DIR/s"
cp -f "$BUILD_DIR/shinobi_interp" "$BOOT_DIR/GoldenAxe"
cat > "$BOOT_DIR/s/startup-sequence" <<'EOF'
; Golden Axe RTG direct boot, based on the supplied RTG/Picasso96 boot disk.
...
HDF="$DIST_DIR/GoldenAxe_RTG.hdf"
rm -f "$HDF"
...
cp -f "$BASE_HDF" "$HDF"
xdftool "$HDF" delete GoldenAxe >/dev/null 2>&1 || true
xdftool "$HDF" delete GoldenAxe.info >/dev/null 2>&1 || true
xdftool "$HDF" delete S/startup-sequence >/dev/null 2>&1 || true
xdftool "$HDF" delete roms >/dev/null 2>&1 || true
xdftool "$HDF" write "$BUILD_DIR/shinobi_interp" GoldenAxe
xdftool "$HDF" write "$BOOT_DIR/s/startup-sequence" S/startup-sequence

if [ -f "$GAME_DIR/roms/goldnaxe3d/bootleg_epr-12525.a7" ]; then
  xdftool "$HDF" makedir roms
  xdftool "$HDF" makedir roms/goldnaxe3d
  xdftool "$HDF" makedir roms/goldnaxe2
  xdftool "$HDF" write "$GAME_DIR/roms/goldnaxe3d/bootleg_epr-12525.a7" roms/goldnaxe3d/bootleg_epr-12525.a7
  xdftool "$HDF" write "$GAME_DIR/roms/goldnaxe3d/bootleg_epr-12524.a5" roms/goldnaxe3d/bootleg_epr-12524.a5
  xdftool "$HDF" write "$GAME_DIR/roms/goldnaxe2/epr-12521.a8" roms/goldnaxe2/epr-12521.a8
  xdftool "$HDF" write "$GAME_DIR/roms/goldnaxe2/epr-12519.a6" roms/goldnaxe2/epr-12519.a6
  xdftool "$HDF" write "$GAME_DIR/roms/epr-12385.ic19" roms/epr-12385.ic19
  xdftool "$HDF" write "$GAME_DIR/roms/epr-12386.ic20" roms/epr-12386.ic20
  xdftool "$HDF" write "$GAME_DIR/roms/epr-12387.ic21" roms/epr-12387.ic21
  xdftool "$HDF" write "$GAME_DIR/roms/mpr-12378.ic9" roms/mpr-12378.ic9
  xdftool "$HDF" write "$GAME_DIR/roms/mpr-12379.ic12" roms/mpr-12379.ic12
  xdftool "$HDF" write "$GAME_DIR/roms/mpr-12380.ic10" roms/mpr-12380.ic10
  xdftool "$HDF" write "$GAME_DIR/roms/mpr-12381.ic13" roms/mpr-12381.ic13
  xdftool "$HDF" write "$GAME_DIR/roms/mpr-12382.ic11" roms/mpr-12382.ic11
  xdftool "$HDF" write "$GAME_DIR/roms/mpr-12383.ic14" roms/mpr-12383.ic14
  xdftool "$HDF" write "$GAME_DIR/roms/epr-12390.ic8" roms/epr-12390.ic8
  xdftool "$HDF" write "$GAME_DIR/roms/mpr-12384.ic6" roms/mpr-12384.ic6
fi

echo "DONE -> $GAME_DIR/GoldenAxe and $HDF"
